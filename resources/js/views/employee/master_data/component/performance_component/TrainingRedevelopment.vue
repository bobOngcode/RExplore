...
export default {
  ...
  methods: {
    ...
    saveTraining() {
      ...
      let api = this.editedTrainingIndex > -1 ? 'update/'+this.editedItem.id : 'store';
      
      axios.post("/api/employee_master_data/training/" + api, data).then(
        (response) => {
          this.loading = false;
    ...
    deleteTraining(item) {
      ...
      this.loading = true;
      let data = { training_id: item.id, employee_id: this.data.id };
