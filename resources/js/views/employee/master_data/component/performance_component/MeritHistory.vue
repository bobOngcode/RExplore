            ...
            <template v-if="index === editedMeritHistoryIndex || item.status === 'New'">
              <td class="pa-2" style="width:40%">
                <v-text-field
                  ...
                  dense
                  hide-details
                  :error-messages="meritDateErrors"
...
export default {
  ...
  methods: {
    ...
    saveMeritHistory() {
      ...
      let api = this.editedMeritHistoryIndex > -1 ? 'update/'+this.editedItem.id : 'store'
      
      axios.post("/api/employee_master_data/merit_history/" + api, data).then(
        (response) => {
          this.loading = false;
    ...
    deleteMeritHistory(item) {
      ...
      this.loading = true;
      let data = { merit_history_id: item.id, employee_id: this.data.id };
