...
export default {
  ...
  methods: {
    ...
    savePerformance() {
      ...
      let api = this.editedPerfomanceIndex > -1 ? 'update/'+this.editedItem.id : 'store';
      
      axios.post("/api/employee_master_data/classroom_performance_rating/" + api, data).then(
        (response) => {
          this.loading = false;
    ...
    deletePerformance(item) {
      ...
      this.loading = true;
      let data = { performance_id: item.id, employee_id: this.data.id };
