...
export default {
  ...
  methods: {
    ...
    saveBranchAssignment() {
      ...
      let api = this.editedBranchAssignmentIndex > -1 ? 'update/'+this.editedItem.id : 'store';
      
      axios.post("/api/employee_master_data/branch_assignment_position/" + api, data).then(
        (response) => {
          this.loading = false;
    ...
    deleteBranchAssignment(item) {
      ...
      this.loading = true;
      let data = { branch_assignment_id: item.id, employee_id: this.data.id };
