        ...
        <v-card v-if="!skeleton_loading">
          ...
          <v-card-title>
            ...
            <template>
              <v-toolbar flat>
                <v-spacer></v-spacer>
                <v-btn color="primary" fab dark class="mb-2" @click="format_date()"
          ...
          <template>
            <v-toolbar class="m-3 mt-5" flat>
              <v-tabs v-model="tab" bg-color="indigo-darken-2" color="black">
                ...
                <v-tab v-for="(item, index) in navigations" :key="index" @click="filtertabs(index)">
                  <h6><b>{{ item }}</b></h6>
                </v-tab>
              </v-tabs>
            </v-toolbar>
          ...
          <v-tabs-items v-model="tab">
            <v-tab-item v-for="(item, index) in navigations" :key="index">
              <v-data-table class="p-3" :headers="headers" :items="table_items" :loading="loading" :search="search">
                      ...
                      <v-row>
                        ...
                        <v-col :cols="12" :md="3">
                          <v-text-field label="Date Submitted *" prepend-icon="mdi-calendar"
                            ...
                            @blur="$v.form_request.date_now.$touch()" :value="formatDate" readonly></v-text-field>
                        </v-col>
              ...
              <template>
                ...
                <v-container fluid class="p-5">
                  <v-row>
                    ...
                    <v-col :cols="12" :md="4" v-if="!form_request.DELETION">
                      <v-text-field v-model="form_request.contact" :readonly="readonly"
                        :error-messages="contactErrors + bankingError.contact"
                        @input="$v.form_request.contact.$touch() + (bankingError.contact = [])"
                        @blur="$v.form_request.contact.$touch()" name="name" @keypress="intNumValFilter()"
                        label="Cellphone # *">
                      </v-text-field>
                    </v-col>

                    <v-col :cols="12" :md="4" v-if="!form_request.DELETION">
                      <v-text-field v-model="form_request.tin" :readonly="readonly"
                        :error-messages="tinErrors + bankingError.tin"
                        @input="$v.form_request.tin.$touch() + (bankingError.tin = [])"
                        @blur="$v.form_request.tin.$touch()" name="name" @keypress="intNumValFilter()" label="TIN # *">
                      </v-text-field>
                    </v-col>

                    <v-col :cols="12" :md="4" v-if="!form_request.DELETION">
                      <v-text-field v-model="form_request.sss" :readonly="readonly"
                        :error-messages="sssErrors + bankingError.sss"
                        @input="$v.form_request.sss.$touch() + (bankingError.sss = [])"
                        @blur="$v.form_request.sss.$touch()" name="name" @keypress="intNumValFilter()" label="SSS # *">
                      </v-text-field>
                    </v-col>

                    <v-col :cols="12" :md="4" v-if="!form_request.DELETION">
                      <v-text-field v-model="form_request.philhealth" :readonly="readonly"
                        :error-messages="philhealthErrors + bankingError.philhealth"
                        @input="$v.form_request.philhealth.$touch() + (bankingError.philhealth = [])"
                        @blur="$v.form_request.philhealth.$touch()" name="name" @keypress="intNumValFilter()"
                        label="PHILHEALTH # *">
                      </v-text-field>
                    </v-col>
                  ...
                  <v-row>
                    ...
                    <v-col :cols="12" v-if="form_request.type === 'ACCOUNT #'" :md="3">
                      <v-text-field v-model="form_request.account_from" :readonly="readonly"
                        ...
                        @blur="$v.form_request.account_from.$touch()" name="number" @keypress="intNumValFilter()"
                        label="From Account # *">
                      </v-text-field>
                    </v-col>
                    ...
                    <v-col :cols="12" v-if="form_request.type === 'ACCOUNT #'" :md="3">
                      <v-text-field v-model="form_request.account_to" :readonly="readonly"
                        ...
                        @blur="$v.form_request.account_to.$touch()" name="number" @keypress="intNumValFilter()"
                        label="To Account # *">
                      </v-text-field>
                    </v-col>
                  ...
                  <v-row>
                    ...
                    <v-col :cols="12" :md="4">
                      <v-text-field v-model="form_request.account_no" :readonly="readonly"
                        ...
                        @blur="$v.form_request.account_no.$touch()" name="account#" @keypress="intNumValFilter()"
                        label="Account # *">
                      </v-text-field>
                    </v-col>
                    ...
                    <v-col :cols="12" :md="4" v-if="!form_request.DELETION">
                      <v-text-field v-model="form_request.amount_limit" :readonly="readonly"
                        ...
                        @blur="$v.form_request.amount_limit.$touch()" name="account#" min="0"
...
export default {
  ...
  methods: {
    ...
    format_date() {
      this.dialog = true;
      this.form_request.date_now = this.formatDate(this.form_request.date_now);
    },
    ...
    getBankingRequest() {
      ...
      axios.get("/api/onlinebanking/index").then(
        (response) => {
          ...
          this.table_items.forEach((item, i) => {
            this.approval_progress.forEach((progress) => {
              if (item.id === progress.banking_request_id) {
                this.table_items[i].approval_progress = progress.progress;
              }
            });
          });
          ...
          this.current_approval_progress()
